...
public class CodeTag {
    ...
    public static void main(String args[]) {
        ...
        String sql = "select Id,Body,Tags from Posts where PostTypeId = 1 and (Tags LIKE '%<c++11>%' OR Tags LIKE '%<visual-c++>%')";//ORDER BY Score DESC  OR Tags LIKE '%<c>%' OR Tags LIKE '%<c++11>%'
        ResultSet rsQuestion = questionOp.search(sql);

        int count = 887602;
        int quesNum = 0;
        try {
            //System.out.println(rsQuestion.getRow());
            //int t = rsQuestion.getRow();
            while (rsQuestion.next()) {
               ...
               // if (quesNum > 385045) {
                    ...
                    if (question != null) {
                        ...
                        System.out.println(count + "-----------" + quesNum+"------------");
                    }
                    //get answers
                    ...
                    while (it.hasNext()) {
                        ...
                        if (currentAnswer != null) {
                            ...
                            //System.out.println(currentAnswer+"**************************************"+quesNum);
                        }
                    }
              //  }
    ...
    public static void insertToDatabase(DatabaseOp questionOp, int num, String text, String tags) {
        //String sqlInsert = "insert into SampleC(Id,Score,Title,Question,Answer1,Answer2,Answer3,Answer4,Answer5,Tags)" +
        //        "values('" + result.get("id") + "', '" + result.get("score") + "','" + result.get("title") + "','" +
        //          result.get("question") + "','" + result.get("answers0") + "','" + result.get("answers1") + "','" + result.get("answers2") +
        //          "','" + result.get("answers3") + "','" + result.get("answers4") + "','" + result.get("tags") + "')";
        String sqlInsert = "insert into SampleC(Id,Code,Tags)" +
                "values('" + num + "','" + text + "', '" + tags + "')";
    ...
    public static LinkedList<String> getAnswer(String id, DatabaseOp questionOp) throws Exception {
        ...
        String sqlFindAnswer = "select Body from Posts where ParentId =" + "'" + id + "'";//+ "ORDER BY Score DESC";
